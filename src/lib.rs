...
impl<O, T: ?Sized> OwningRef<O, T> {
    ...
    ///     // create a owning reference that points at the
    ///     // third element of the array.
    ///     let owning_ref = owning_ref.filter_map(|array| (array[2] == 3).then(|| &array[2]));
    ...
    ///     // second element of the array from the owning ref that was pointing to the third
    ///     let owning_ref =
    ///         owning_ref.filter_map_with_owner(|array, _prev| (array[1] == 2).then(|| &array[1]));
...
impl<O, T: ?Sized> OwningRefMut<O, T> {
    ...
    ///         owning_ref_mut_a.erase_owner(),
    ///         owning_ref_mut_b.erase_owner(),
    ///     ];
    ///
...
mod tests {
    mod owning_ref {
        ...
        fn raii_locks() {
            use super::super::{
                MutexGuardRef, RefMutRef, RefRef, RwLockReadGuardRef, RwLockWriteGuardRef,
            };
            use std::{
                cell::RefCell,
                sync::{Mutex, RwLock},
    ...
    mod owning_ref_mut {
        use super::super::{BoxRef, BoxRefMut, Erased, ErasedBoxRefMut, OwningRefMut};
        use std::{
            cmp::{Ord, Ordering, PartialEq, PartialOrd},
            collections::{hash_map::DefaultHasher, HashMap},
            hash::{Hash, Hasher},
        ...
        #[test]
        fn raii_locks() {
            use super::super::{MutexGuardRefMut, RefMutRefMut, RwLockWriteGuardRefMut};
            use std::{
                cell::RefCell,
                sync::{Mutex, RwLock},
            };
